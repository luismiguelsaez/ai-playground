...
- Check Redis pods again for current master

- After confirming the failover logs, check the Redis operator logs to verify it detected the failover too

- Check pods errors in the same namespace for Redis related errors, like `notifications-hub` pods. Compare the errors to the failover timestamps
